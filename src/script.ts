...
const EDIT_DIALOG = document.getElementById(
	"taskEditDialog",
) as HTMLDialogElement;
const EDIT_TITLE_FIELD = document.getElementById(
	"editTitle",
) as HTMLInputElement;
const EDIT_DESC_FIELD = document.getElementById(
	"editDescription",
) as HTMLTextAreaElement;
const EDIT_DUE_DATE_FIELD = document.getElementById(
	"editDueDate",
) as HTMLInputElement;
const EDIT_PRIORITY_FIELD = document.getElementById(
	"editPrioritySelect",
) as HTMLSelectElement;
const EDIT_ID_FIELD = document.getElementById("editId") as HTMLInputElement;
const ENTRY_TITLE_FIELD = document.getElementById(
	"entryTitle",
) as HTMLInputElement;
const ENTRY_DESC_FIELD = document.getElementById(
	"entryDescription",
) as HTMLTextAreaElement;
const ENTRY_DUE_DATE_FIELD = document.getElementById(
	"entryDueDate",
) as HTMLInputElement;
const ENTRY_PRIORITY_FIELD = document.getElementById(
	"entryPrioritySelect",
) as HTMLSelectElement;
const PROJECT_SELECT = document.getElementById(
	"projectSelect",
) as HTMLSelectElement;
const ADD_PROJECT_DIALOG = document.getElementById(
	"addProjectDialog",
) as HTMLDialogElement;
const OPEN_ADD_PROJECT_BUTTON = document.getElementById(
	"openAddProject",
) as HTMLButtonElement;
const ADD_PROJECT_NAME = document.getElementById(
	"addProjectName",
) as HTMLInputElement;
...
OPEN_ADD_PROJECT_BUTTON.onclick = () => {
	...
	if (event.target instanceof Element) {
		...
		if (event.target.id === "entryForm") {
			const formFields = Array.from(
				document.querySelectorAll("#entryForm *[id]"),
			) as HTMLInputElement[];
		...
		//if the edit form was submitted, modify the task existing at the provided index
			...
			const taskToEdit = taskList.getTask(Number.parseInt(EDIT_ID_FIELD.value));
...
addEventListener("taskEdit", (event: CustomEvent) => {
	...
	EDIT_DUE_DATE_FIELD.value = event.detail.task.dueDate
		.toISOString()
		.substring(0, 10);
